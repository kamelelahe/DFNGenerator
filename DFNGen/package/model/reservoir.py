...
class UnstructReservoir:
    def __init__(self, permx, permy, permz, frac_aper, mesh_file, poro, bound_cond, physics_type,
        ...
        self.min_depth = max(self.unstr_discr.depth_all_cells[:self.nb])
        ...
        ##calcuate right and left boudaries:
        ...
        #print('self.left_boundary_cells', self.left_boundary_cells)
        #print('self.right_boundary_cells', self.right_boundary_cells)
        # Calculate boundary cells using the calc_boundary_cells method:
        self.calc_boundary_cells1(boundary_data)
        ...
        # Define the boundary_data dictionary
        boundary_data = {
            'first_boundary_dir': 'Y',
            'first_boundary_val': np.min(self.unstr_discr.mesh_data.points[:, 1]),
            'second_boundary_dir': 'Y',
            'second_boundary_val': np.max(self.unstr_discr.mesh_data.points[:, 1])
        }
        # Call the calc_boundary_cells method
        ...
        if bound_cond == 'const_pres_rate':
            ...
            for ith_inj in dummy_well_inj:
                dist_to_well_point = np.linalg.norm(
                    self.unstr_discr.centroid_all_cells[left_int:right_int] - ith_inj,
                    axis=1)
            ...
            for ith_prod in dummy_well_prod:
                dist_to_well_point = np.linalg.norm(
                    self.unstr_discr.centroid_all_cells[left_int:right_int] - ith_prod,
                    axis=1)
                cell_id = np.argmin(dist_to_well_point) + offset
    ...
    def calc_boundary_cells(self, boundary_data):
        ...
        # Helper function to get boundary points
        def get_boundary_points(direction, value):
            if direction == 'X':
                return np.array(list(compress(range(len(self.unstr_discr.mesh_data.points[:, 0])),
                                              self.unstr_discr.mesh_data.points[:, 0] == value)))
            elif direction == 'Y':
                return np.array(list(compress(range(len(self.unstr_discr.mesh_data.points[:, 1])),
                                              self.unstr_discr.mesh_data.points[:, 1] == value)))
            elif direction == 'Z':
                return np.array(list(compress(range(len(self.unstr_discr.mesh_data.points[:, 2])),
                                              self.unstr_discr.mesh_data.points[:, 2] == value)))

        # Helper function to get boundary cells
        def get_boundary_cells(boundary_points):
            boundary_cells = {}
            count = 0
            for geometry in self.unstr_discr.geometries_in_mesh_file:
                if geometry in self.unstr_discr.available_matrix_geometries:
                    for ith_cell, ith_row in enumerate(self.unstr_discr.mesh_data.cells_dict[geometry]):
                        if len(set.intersection(set(ith_row), set(boundary_points))) == 4:
                            boundary_cells[count] = ith_cell
                            count += 1
            return np.array(list(boundary_cells.values()), dtype=int) + self.unstr_discr.fracture_cell_count

        # Calculate left boundary cells
        left_boundary_points = get_boundary_points(boundary_data['first_boundary_dir'],
                                                   boundary_data['first_boundary_val'])
        self.left_boundary_cells = get_boundary_cells(left_boundary_points)

        # Calculate right boundary cells
        right_boundary_points = get_boundary_points(boundary_data['second_boundary_dir'],
                                                    boundary_data['second_boundary_val'])
        self.right_boundary_cells = get_boundary_cells(right_boundary_points)

        # Calculate top boundary cells
        top_boundary_points = get_boundary_points('Y', np.max(self.unstr_discr.mesh_data.points[:, 1]))
        self.top_boundary_cells = get_boundary_cells(top_boundary_points)

        # Calculate bottom boundary cells
        bottom_boundary_points = get_boundary_points('Y', np.min(self.unstr_discr.mesh_data.points[:, 1]))
        self.bottom_boundary_cells = get_boundary_cells(bottom_boundary_points)

        return 0

    def calc_boundary_cells1(self, boundary_data):
        """
