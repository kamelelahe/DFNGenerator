...
size_report_step = 0.001  # Size of the reporting step (when output is writen to .vtk format)
...
def main():
    ...
    for dfn_file in dfn_files:
            ...
            for char_len in char_len_list:
                for val in paramList:
                    ...
                    output_dir_mesh = 'DFNs/' + str(
                        DFN_name) + '/' + mesh_folder + '/' + DFN_textName + '/Processsed' + str(char_len)
                    print('---', output_dir_mesh, '----')
                    mesh_name = DFN_textName + '_raw_lc_' + str(32) + '.msh'
        ...
        v = Visualize(results=results, paramList=paramList, paramName=str(paramName),
                      labels=['CharacteristicLength', str(paramName), DFN_textName], output_dir=outputDir,
                      inputs=inputs)
