...
class generateConjugateFractures:
    ...
    def __init__(self, domainLengthX, domainLengthY, sets, fractureLengthPDF, fractureLengthPDFParams,
                 spatialDisturbutionPDF, spatialDisturbutionPDFParams, outputDir, stressAzimuth=None, savePic=True):
        ...
        for set in sets:
            print(set)
            fractureSet = generateFractures(self.xmax,self.ymax,set['I'],set['theta'],self.nameFractureLengthPDF,fractureLengthPDFParams)
            fractureSet = sortFractures(fractureSet)
            fractureSet = self.computeSpacing(fractureSet)
            allFractureSets.append(fractureSet)

        print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('+++++++ Section B: placing the fractures +++++++++')
        print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        allProcessedFractureSets=[]
        for set in allFractureSets:
            processedFractureSet = self.place_fractures(set, thetaYforGeneration)
        ...
        self.rawVerticalFractures= self.generateFractures(Iy,thetaYforGeneration,nameFractureLengthPDF,fractureLengthPDFParams)
    ...
    def generateFractures(self,fractureIntensity,theta,nameFractureLengthPDF,fractureLengthPDFParams):
        """
        Parameters:
        ...
        print('theta=',thetaYforGeneration)
