...
import math
import os
from .apertureCalculation import apertureCorrelation
from matplotlib.colors import Normalize
from matplotlib.cm import get_cmap

class generateConjugateFractures:

    def __init__(self, domainLengthX, domainLengthY, sets, outputDir, stressAzimuth=None, rockType=None,depth=1000 ,savePic=True):
        ...
        #self.thetaValues = [s['theta'] for s in sets]
        ...
        for setConfig in sets:
            ...
            fractureSet = self.computeSpacing(fractureSet)
            fractureSet = self.computeAperture_SubLinearLengthApertureScaling(fractureSet)
        ...
        if stressAzimuth is not None:
            ...
            for azimuth in stressAzimuth:
                for set in allProcessedFractureSets:
                    correlatedSet = apertureCorrelation(set, rockType ,S_Hmax, S_hmin, S_v,azimuth).fractures
                    set=correlatedSet
            #self.plotApertureChanges(allProcessedFractureSets,self.thetaValues ,stressAzimuth ,'apertureChange')
            #self.plotStereographic(self.thetaValues , stressAzimuth,'stratigraphy')
        ...
        if savePic:
            self.plotFractures(allProcessedFractureSets,name= 'DFNPic',stressAzimuth= stressAzimuth)
    ...
    def place_fractures(self,fractures, setConfig):
        ...
        spatialDisturbutionPDF = PDFs(setConfig['spatialDisturbutionPDF'], setConfig['spatialDisturbutionPDFParams'])
    ...
    def generateFractures(self,setConfig):
        ...
        orientationDisturbutonPDF=PDFs(setConfig['orientationDisturbutonPDF'], setConfig['orientationDisturbutonPDFParams'])
        ...
        fractureLengthPDF = PDFs(setConfig['fractureLengthPDF'], setConfig['fractureLengthPDFParams'])
        print('appended')
        fractures=[]
        newFrac={}
    ...
    def plotFractures(self, fractureSets, stressAzimuth, name, number=0):
        ...
        for fractures, color, label in zip(fractureSets, colors, setLabels):
            for fracture in fractures:
                ...
                plt.plot([x_start, x_end], [y_start, y_end], color=color,
                         linewidth=fracture['fracture aperture'] * 1000, label=label)
    ...
    def generateOutputFileForOutputPropertiesPerSet(self, name, fractureSets, number=0):
        ...
        with open(outputPropertiesFile, 'w') as fileID:
            for i, fractures in enumerate(fractureSets):
                ...
                self.computeAperture_SubLinearLengthApertureScaling(fractures)
    ...
    def plotApertureChanges(self, fractureSets, thetaValues, stressAzimuth, name, number=0):
        ...
        for azimuth in stressAzimuth:
            for theta, fractures in zip(thetaValues, fractureSets):
                ...
                avg_aperture_before = np.mean([frac['fracture aperture'] for frac in fractures])

                # Retrieve the average aperture after deformation for the current azimuth
                avg_aperture_after = np.mean([frac['correctedAperture' + str(azimuth)] for frac in fractures])
        ...
        num_azimuths = len(stressAzimuth)
        ...
        ax.set_xticks(np.arange(len(stressAzimuth)) * num_sets-0.5)
    ...
    def plotStereographic(self,thetaValues, shmax_azimuths,name,number=0):
        figDir = self.outputDir + '/stressDependancy'
        os.makedirs(figDir, exist_ok=True)
        figDirFile = f"{figDir}\\{name}{number + 1:03}.png"
